...
filtered_id AS(
    ...
    WHERE 
        ...
        AND ((ceiling_height < (SELECT ceiling_height_limit_h FROM limits)
            AND ceiling_height > (SELECT ceiling_height_limit_l FROM limits)) OR ceiling_height IS NULL)
            AND type_id = 'F8EM' -- ✅️ добавила фильтр по типу город (можно присоединить таблицу, но я разово посмотрела идентификатор города)
),  
...
SELECT 
 ...
 CASE 
  ...
  ELSE 'активные' -- ✅️ добавила вывод активных объявлений
 END AS times_case, 
...
-- ✍️ Меняла значение a.first_day_exposition + (a.days_exposition * INTERVAL '1 day') на a.first_day_exposition при получении данных на момент 
-- публикации и покупки, не стала делать одним запросом, так как если я верно поняла, ключевая цель - не супер красивый запрос написать, а дать 
...
-- ✍️ В зависимости от вопроса сортировала нужные мне столбцы, чтобы проще анализировать данные